...
public class AnxietyBar : MonoBehaviour
{
    ...
    //Change the position and alpha of the overlay over time
    void OverlayState()
    {
        newColour.a = anxietyBar.value;

        overlays[0].rectTransform.position = Vector3.Lerp(startPoss[0], new Vector3(startPoss[0].x - 400, startPoss[0].y, 0), anxietyBar.value);
        overlays[1].rectTransform.position = Vector3.Lerp(startPoss[1], new Vector3(startPoss[1].x + 400, startPoss[1].y, 0), anxietyBar.value);
    ...
    void ChangeSpeed()
    {
        ...
        //Change animator paramaters values based on the anxietr bar's value
        if (anxietyBar.value >= 0.95f)
        {
            playerSpeed = 0.0f;
            GameObject.FindGameObjectWithTag("Player").GetComponent<Animator>().SetBool("runOnce", true);
        }
        else
            GameObject.FindGameObjectWithTag("Player").GetComponent<Animator>().SetBool("runOnce", false);
    ...
    //Call to set how far the anxiety bar should move
    public void NarrativeAnxiety(float stoppingPoint)
    {
        anxietyBar.value = Mathf.Clamp(anxietyBar.value, 0.0f, stoppingPoint);
    }
}
